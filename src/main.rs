...
use crate::graph::{ComputationGraph, Subgraph, TensorId};
use crate::input_format::{DeviceParameters, InputFormat};
use crate::local_optimization::{Partition, partition_subgraph};
...
fn main() {
    ...
    if cli.compare_naive {
        ...
        if optimized_total > 0.0 {
            eprintln!("Speedup:                 {:.2}x", naive_total / optimized_total);
        }
    }
}
